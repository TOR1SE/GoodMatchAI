  ...
  methods: {
    ...
    validateLoginForm() {
      ...
      if (!this.loginForm.captcha.trim()) {
        ...
        isValid = false;
      } else if (this.loginForm.captcha.toUpperCase() !== this.captchaText) {
        this.errors.captcha = '验证码错误，请重新输入';
        isValid = false;
    ...
    async handleLogin() {
      ...
      try {
        ...
        // 调用登录接口
        const response = await login(this.loginForm.account, this.loginForm.password);
        ...
        alert(error.message || '登录失败，账号或密码错误。请使用演示账号体验：账号: demo, 密码: demo123');
        this.refreshCaptcha();
      } finally {
        this.loading = false;
      }
